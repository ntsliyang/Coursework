...
class flatten(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = dprev.reshape(dprev.shape[0], np.prod(dprev.shape[1:]))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class fc(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        output = np.dot(feat.reshape(feat.shape[0], np.prod(feat.shape[1:])), self.params[self.w_name]) + self.params[self.b_name]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        feat = feat.reshape(feat.shape[0], np.prod(feat.shape[1:]))
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True):
        ...
        if is_training and self.keep_prob > 0:
            kept = (np.random.random_sample(feat.shape) < self.keep_prob) / self.keep_prob
            output = feat * kept
        else:
            output = feat
        #############################################################################
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        loss = - np.sum(np.log(logit[range(label.shape[0]), label])) 
    ...
    def backward(self):
        ...
        #############################################################################
        dlogit = self.logit
        dlogit[range(label.shape[0]), label] -= 1
