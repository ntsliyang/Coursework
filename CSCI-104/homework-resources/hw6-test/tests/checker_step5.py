...
def step5(problem):
    ...
    subprocess.call(['python', 'data_gen.py'])
    ...
    try:
        stress_file = open(output_files[-1], 'r')
        stress_array = stress_file.readlines()
        stress_file.close()
        fix_file = open(output_files[-1], 'w')
        for i in range(0, 6):
            fix_file.write(stress_array[i])
        fix_file.close()
    except:
        pass
    ...
    subprocess.call(['python', 'data_rm.py'])
