...
void PageRank(set<WebPage *> &CandSet, map<string, WebPage *> &m, double probability, int n, ofstream& output)
{
	...
	for (auto i = CandSet.begin(); i != CandSet.end(); ++i) {
		vector<string> outlinks = (*i)->getOutlinks();
		outlinks.push_back((*i)->getName());
		(*i)->setOutlinks(outlinks);
		vector<string> inlinks = (*i)->getInlinks();
		inlinks.push_back((*i)->getName());
		(*i)->setInlinks(inlinks);
	}

	// initialize the probability for all the nodes in candidate set
	for (auto i = CandSet.begin(); i != CandSet.end(); ++i)
	{
		(*i)->setOldWeight(1.0 / n);		
	}	
	...
	for (auto k = 0; k < n; k++)
	{
		for (auto i = CandSet.begin(); i != CandSet.end(); ++i)
		{
			...
			{
				inWeight += (m[*j])->getOldweight() / m[*j]->getOutlinks().size();
			}
			temp += (1 - probability) * inWeight + probability * (1 / n);
			(*i)->setNewWeight(temp);
...
{
	map<string, string> config = loadConfig(argv[1]);
