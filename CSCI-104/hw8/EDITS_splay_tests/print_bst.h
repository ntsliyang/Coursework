...
int getNodeDepth(BinarySearchTree<Key, Value> & tree, Node<Key, Value> * node)
{
	int dist = 1;
...
/* Function to prettily print a BST out to the terminal.
   Enough of this "giant glob of parenthesis" ugliness!
   We need to show people what their tree actually looks like!
...
template<typename Key, typename Value>
void prettyPrintBST(BinarySearchTree<Key, Value> & tree)
{
...
#define ELEMENT_WIDTH (BOX_WIDTH + PADDING)
	...
	// ----------------------------------------------------------------------
	uint32_t printedTreeHeight = getSubtreeHeight(tree.mRoot);
	...
	//std::cout << "finalRowNumElements = " << finalRowNumElements << ", finalRowWidth = " << finalRowWidth << std::endl;
	...
	for(typename BinarySearchTree<Key, Value>::iterator treeIter = tree.begin(); treeIter != tree.end(); ++treeIter)
	{
		//std::cout << "height of node " << treeIter->first << ": " << getDistFromRoot(tree, treeIter.mCurrent) << std::endl;

		if(getNodeDepth(tree, treeIter.mCurrent) != -1)
		{
	...
	currRowNodes.push_back(tree.mRoot);

	for(size_t levelIndex = 0; levelIndex < printedTreeHeight; ++levelIndex)
	{
		uint16_t numElements =(uint16_t)std::pow(2, levelIndex);

		// number of spaces between each element
		//uint16_t elementSpacing = (finalRowWidth) / (numElements + 1) - BOX_WIDTH / 2;

		//std::cout << "firstElementMargin = " << firstElementMargin << ", elementPadding = " << elementPadding << std::endl;
		...
		for(size_t elementIndex = 0; elementIndex < numElements; ++elementIndex)
		{
			...
			{
				uint16_t placeholder = valuePlaceholders[currRowNodes[elementIndex]->mItem.first];
		...
		for(typename std::vector<Node<Key, Value> *>::iterator prevRowIter = prevRowNodes.begin(); prevRowIter != prevRowNodes.end() ;++prevRowIter)
		{
			...
			{
				currRowNodes.push_back((*prevRowIter)->mLeft);
				currRowNodes.push_back((*prevRowIter)->mRight);
			}
		}
		...
		if(levelIndex < printedTreeHeight - 1)
		{
			...
			for(size_t prevRowElementIndex = 0; prevRowElementIndex < prevRowNodes.size(); ++prevRowElementIndex)
			{
				...
				// print first branch
				if(currNode == nullptr || currNode->mLeft == nullptr)
				{
				...
				// print second branch
				if(currNode == nullptr || currNode->mRight == nullptr)
				{
					std::cout << std::string(elementPadding/2 + 3, ' ');
				}
				else
	...
	if(!std::is_same<Key, uint8_t>::value) // print placeholder explanations if needed:
	{
		...
		for(typename std::map<Key, uint8_t>::iterator placeholdersIter = valuePlaceholders.begin(); placeholdersIter != valuePlaceholders.end(); ++placeholdersIter)
		{
			...
			typename BinarySearchTree<Key, Value>::iterator elementIter = tree.find(placeholdersIter->first);
			if(elementIter == tree.end())
			{
